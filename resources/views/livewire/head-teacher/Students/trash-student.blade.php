    ...
    @if($student->trash == false)
         {{-- Counting the trash to see if student id dont exist in there, if the count is less than one then we show this --}}
            <div class="row">
                ...
                <button class="btn btn-danger col-6" wire:click='trashStudent({{ $student->id  }})'> Delete {{ $student->_firstName }} </button>
            </div>

            <a class="btn btn-primary col-6" href="{{ route('studentFormEdit', $student->_firstName) }}"> make changes to {{ $student->_firstName }} </a>
